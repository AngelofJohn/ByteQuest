...
// =====================================================
// Quest System - Using QuestManager
// =====================================================
...
function updateQuestProgress(questId, objectiveId, increment = 1) {
  if (questManager) {
    questManager.updateObjective(questId, objectiveId, increment);
    renderQuestPanel();
  }
}
...
function reviewLessons() {
  ...
  GameState.lessonState = {
    ...
    totalBonusXP: 0,
    totalBonusGold: 0,
    isReview: true // Flag this as a review session
  };
...
function interactWithNPC(npcId) {
  ...
  const availableQuest = questManager.getAvailableQuests().find(q => q.giver === npcId);
  if (availableQuest) {
    dialogText = GAME_DATA.quests[availableQuest.id].dialogue.intro;
    options.push({
      text: "Accept Quest",
      action: () => {
        handleAcceptQuest(availableQuest.id);
        hideDialog();
      }
    });
    options.push({
    ...
    if (activeQuest) {
      ...
      if (allComplete) {
        ...
        options.push({
          text: "Complete Quest",
          action: () => {
            handleCompleteQuest(activeQuest.id);
            hideDialog();
          }
        });
      } else {
