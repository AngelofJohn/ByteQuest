...
const MILESTONE_DEFINITIONS = {
  ...
  [MilestoneType.ACCURACY]: {
    ...
    icon: '🎯',
    tiers: [
      { threshold: 70, reward: { stat: StatType.INSIGHT, amount: 1 }, label: 'Careful' },
      { threshold: 80, reward: { stat: StatType.INSIGHT, amount: 1 }, label: 'Precise' },
      { threshold: 90, reward: { stat: StatType.INSIGHT, amount: 1 }, label: 'Masterful' }
    ],
    getValue: (state) => {
...
const ACHIEVEMENT_DEFINITIONS = {
  ...
  friendly_face: {
    ...
    reward: { stat: StatType.INSIGHT, amount: 1 },
    check: (state) => {
      // All non-hidden Dawnmere NPCs (excludes old_pieron who is hidden)
      const dawnmereNpcs = [
        'urma', 'rega', 'merchant', 'baker', 'sage_aldric',
